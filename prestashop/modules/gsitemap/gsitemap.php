...
class Gsitemap extends Module
{
	...
	public function getContent()
	{
		...
		$this->context->smarty->assign(array(
			...
			'gsitemap_feed_exists' => file_exists(dirname(__FILE__).'/../../sitemap.xml'),
	...
	public function createSitemap($id_shop = 0)
	{
		...
		Tools::file_get_contents('http://www.google.com/webmasters/sitemaps/ping?sitemap='.urlencode('http'.(Configuration::get('PS_SSL_ENABLED') ? 's' : '').'://'.Tools::getShopDomain(false, true).$this->context->shop->physical_uri.$this->context->shop->virtual_uri.$this->context->shop->id.'sitemap.xml'));
	...
	private function _createIndexSitemap()
	{
		...
		file_put_contents(dirname(__FILE__).'/../../sitemap.xml', $xml_feed->asXML());
	}
